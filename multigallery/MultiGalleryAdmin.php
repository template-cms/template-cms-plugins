<?php
    ...
    addHook('admin_themes_extra_template_actions','MultiGalleryFormComponent'); // @Awilum
    addHook('admin_themes_extra_actions','MultiGalleryFormComponentSave'); // @Awilum

    /**
     * MultiGallery admin
     */
    function MultiGalleryAdmin() {
          $ext = '.xml';

       // Images path
        $images_path = '../'.TEMPLATE_CMS_DATA_PATH.'multigallery/';

		//directory list of files list
		$dir_mode=true;

        // Get current gallery
        if (isGet('gal')) {
        	$current_path=get('gal');
        	$dir_mode=true;
        }
        	else   $current_path="default";

        // Images thumbs path
        $images_thumbs_path = '../'.TEMPLATE_CMS_DATA_PATH.'multigallery/'.$current_path.'/thumbs/';


        // Gallery config folder
        $MultiGallery_config = '../'.TEMPLATE_CMS_DATA_PATH.'multigallery/'.$current_path.'/config/';

        if(is_dir($images_path)) {
             if(!is_dir($images_path.$current_path)) {
                mkdir($images_path.$current_path, 0755);
               createXMLdb($images_path.$current_path.'/MultiGallery_image_info');
                $xml_db = getXMLdb($images_path.$current_path.'/MultiGallery_image_info'.$ext);
            insertXMLRecord($xml_db, 'gallery_nasv',array('nasv'=>'new gallery'));
            }
            else {
               if(file_exists($images_path.$current_path.'/MultiGallery_image_info'.$ext)) {
                    //$xml_db = getXMLdb($images_path.$current_path.'/MultiGallery_image_info'.$ext);
                } else {
                    createXMLdb($images_path.$current_path.'/MultiGallery_image_info');
                    //$xml_db = getXMLdb($MultiGallery_config.'MultiGallery_config'.$ext);
                }
            }
            ...
            if(!is_dir($MultiGallery_config)) {
                mkdir($MultiGallery_config, 0755);

               createXMLdb($MultiGallery_config.'MultiGallery_config');
               $xml_db = getXMLdb($MultiGallery_config.'MultiGallery_config'.$ext);
                ...
                if(file_exists($MultiGallery_config.'MultiGallery_config'.$ext)) {
                    $xml_db = getXMLdb($MultiGallery_config.'MultiGallery_config'.$ext);
                } else {
                    createXMLdb($MultiGallery_config.'MultiGallery_config');
                    $xml_db = getXMLdb($MultiGallery_config.'MultiGallery_config'.$ext);
        ...
        /**
         * Get records from MultiGallery config database
         */
         if($xml_db) {
     	 ...
     	 /**
         * Upload image to MultiGallery folder and create thumb
         */
        if(isPost('upload_image')) {
            if ($_FILES['file']) {
                if($_FILES['file']['type'] == 'image/jpeg' ||
                   ...
                   move_uploaded_file($_FILES['file']['tmp_name'],$images_path.$current_path."/".$_FILES['file']['name']);
                   $k=$_FILES['file']['name'];
                   //echo $images_path.$current_path."/".$_FILES['file']['name'];
                   createThumb($images_path.$current_path."/".$_FILES['file']['name'], $MultiGallery_config_xml[0]->thumbnail_width, $MultiGallery_config_xml[0]->thumbnail_height,$_FILES['file'],$images_path.$current_path."/".'thumbs/'.$_FILES['file']['name'], 100, true);
                    createDesc('new record',$current_path,$k);
			     }
            }
        ...
        if(isPost('save_desc')) {
            ...
            $xml_db2 = getXMLdb($images_path.$current_path."/".'MultiGallery_image_info'.$ext);
            $query="//item[filename='".post('filename')."']";
            updateXMLRecordWhere($xml_db2,$query, array('desc'=>post('desc')));
         ...
         * Save galery options
         */
        if(isPost('MultiGallery_save_options')) {

            updateXMLRecord($xml_db, 'option', 1, array('thumbnail_width'=>(int)post('thumbnail_width'),
        ...
        if (isGet('action')) {
            ...
            switch (get('action')) {
                case "delete":
                    deleteFile($images_path.$current_path."/".get('image'),$current_path);
                    deleteFile($images_thumbs_path.get('image'),$current_path);
                    deleteDesc(get('image'),$current_path);
                    redirect('index.php?id=pages&sub_id=multigallery');
                break;
               case "delete_folder":

               deleteDir($images_path.get('gal'));
               redirect('index.php?id=pages&sub_id=multigallery');
               break;
               case "create_folder":
               redirect('index.php?id=pages&sub_id=multigallery&gal='.post('new_name'));
               break;
               case "edit_folder":
               redirect('index.php?id=pages&sub_id=multigallery&gal='.get('gal'));
               break;
            }
        } else { // Load main template
            $images = array();
            $images = listFiles('../'.TEMPLATE_CMS_DATA_PATH.$current_path.'/thumbs/');
            $folders= listOfFolders('../'.TEMPLATE_CMS_DATA_PATH.'multigallery/'); //@Awilum MultiGallery -> multigallery


            if (!$dir_mode)   include 'templates/backend/MultiGalleryTemplate.php';
                else  include 'templates/backend/MultiGalleryTemplateList.php';
   ...
   function createThumb($src_file, $max_w, $max_h, &$image_info, $dst_file = null, $quality = 100, $overwrite = true) {
     ...
     */
    function MultiGalleryFormComponentSave() {
        if(isPost('MultiGallery_component_save')) {
            // Prepare content before saving
            $content = '<?xml version="1.0" encoding="UTF-8"?>';
            $content .= '<root>';
            $content .= '<template>'.post('MultiGallery_form_template').'</template>';
            $content .= '</root>';

            createFile('../'.TEMPLATE_CMS_DATA_PATH.'other/MultiGallery_template.xml',$content);
     ...
     */
    function MultiGalleryFormComponent() {
        ...
        htmlSelect($templates, array('style'=>'width:200px;','name'=>'MultiGallery_form_template'), lang('MultiGallery_name'), $template_xml->template);
        htmlNbsp();
        htmlFormClose(true, array('value'=>lang('themes_save'),'name'=>'MultiGallery_component_save'));
    }
 ...
 function createDesc($desc,$current_path='default',$filename) {
		$ext = '.xml';
      ...
      $xml_db2 = getXMLdb($images_path.'MultiGallery_image_info'.$ext);

      insertXMLRecord($xml_db2, 'item', array('filename'=>$filename,
                    'desc'=>$desc));
 	}
 ...
 function deleteDesc($filename,$current_path){
  $ext = '.xml';
       ...
       $xml_db3 = getXMLdb($images_path.'MultiGallery_image_info'.$ext);
   	   $query="//item[filename='$filename']";
	   deleteXMLRecordwhere($xml_db3, $query);
 }
